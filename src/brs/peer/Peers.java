...
public final class Peers {
  ...
  public static boolean isSupportedUserAgent(String header) {
    if(header == null || header.isEmpty() || ! header.trim().startsWith("BFC/")) {
      return false;
    } else {
      try {
        return isHigherOrEqualVersion(MIN_VERSION, Version.parse(header.trim().substring("BFC/".length())));
  ...
  static final int DEFAULT_PEER_PORT = 8323;
  ...
  public static Peer getAnyPeer(Peer.State state) {
    if(!connectWellKnownFinished) {
      ...
      if (wellKnownConnected >= connectWellKnownFirst) {
        ...
        logger.info("You can open your Burst Wallet in your favorite browser with: http://127.0.0.1:8325 or http://localhost:8325");
      }
    }
