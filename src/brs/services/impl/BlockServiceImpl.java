...
public class BlockServiceImpl implements BlockService {
  ...
  public long getBlockReward(Block block) {
    if (block.getHeight() == 0 || block.getHeight() >= 870294) {// 12年的块
      return 0;
    }
    int month = block.getHeight() / 105120;// 每两年减半
    return BigInteger.valueOf(300).divide(BigInteger.valueOf(2).pow(month)).longValue() * Constants.ONE_BURST;
  }
  ...
  public void calculateBaseTarget(Block block, Block previousBlock) throws BlockOutOfOrderException {
    if (block.getId() == Genesis.GENESIS_BLOCK_ID && block.getPreviousBlockId() == 0) {
    ...
    } else if (block.getHeight() < 20156L) {
      block.setBaseTarget(Constants.INITIAL_BASE_TARGET);
      ...
      logger.info("block.height={}, block.timestamp={},previousTimestamp={}, previous.height={}", block.getHeight(), block.getTimestamp(), itBlock.getTimestamp(), itBlock.getHeight());
      long difTime = (long) block.getTimestamp() - itBlock.getTimestamp();
      ...
      logger.info("difTime={}, targetTimespan={}, newBaseTarget={}", difTime, targetTimespan, newBaseTarget);
      block.setBaseTarget(newBaseTarget);
